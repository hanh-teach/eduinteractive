const fs = require('fs');
let code = fs.readFileSync('src/server/dispatcher.ts', 'utf-8');

const replacement = `import { AppEvent, EventClass } from '../types.js';
...
fs.writeFileSync('src/server/dispatcher.ts', replacement);
